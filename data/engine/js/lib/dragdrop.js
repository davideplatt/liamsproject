...
var Droppables = {drops: [], delayedDroppoffs: [], remove: function(a) {
    ...
    Position.prepare();
    if (!Object.isUndefined(c.changedTouches)) {
        var a = c.changedTouches[0].clientX;
        var d = c.changedTouches[0].clientY
    } else {
        var a = Event.pointerX(c);
        var d = Event.pointerY(c)
    }
    if (this.isAffected([a, d], b, this.last_active)) {
        if (this.last_active.onDrop) {
            this.last_active.onDrop(b, this.last_active.element, c);
            return true
        }
    }
}, reset: function() {
...
}, prepareDelayedDropOff: function(a, b) {
    Droppables.delayedDroppoffs.push({element: a, event: b})
}, haveDelayedDropOffs: function() {
    return(Droppables.delayedDroppoffs.length > 0) ? true : false
}, runDelayedDropOffs: function() {
    Droppables.delayedDroppoffs.each(function(a) {
        var b = [Event.pointerX(a.event), Event.pointerY(a.event)];
        if (!b[0] && !Object.isUndefined(a.event.touches[0])) {
            var b = [a.event.touches[0].clientX, a.event.touches[0].clientY]
        }
        if (!b[0] && !Object.isUndefined(a.event.changedTouches)) {
            var b = [a.event.changedTouches[0].clientX, a.event.changedTouches[0].clientY]
        }
        if (window.pageXOffset) {
            b[0] += window.pageXOffset
        }
        if (window.pageYOffset) {
            b[1] += window.pageYOffset
        }
        Droppables.show(b, a.element);
        if (Droppables.last_active) {
            Droppables.last_active.onDrop(a.element, Droppables.last_active.element, a.event);
            Droppables.reset()
        }
    });
    Droppables.delayedDroppoffs = []
}};
var Draggables = {drags: [], observers: [], register: function(a) {
    if (this.drags.length == 0) {
        this.eventMouseUp = this.endDrag.bindAsEventListener(this);
        this.eventMouseMove = this.updateDrag.bindAsEventListener(this);
        this.eventKeypress = this.keyPress.bindAsEventListener(this)
    }
    if (a.element.tagName.toLowerCase() == "object") {
        if (document.ontouchstart === undefined) {
            Event.observe(a.element, "mouseup", this.eventMouseUp);
            Event.observe(document, "mousemove", this.eventMouseMove)
        } else {
            Event.observe(a.element, "touchend", this.eventMouseUp);
    ...
    if (!b[0] && !Object.isUndefined(a.touches[0])) {
        var b = [a.touches[0].clientX, a.touches[0].clientY]
    }
...
var Draggable = Class.create({initialize: function(b) {
    var c = {handle: false, reverteffect: function(f, e, d) {
        ...
        new Effect.Opacity(e, {duration: 0.2, from: 0.7, to: d, queue: {scope: "_draggable", position: "end"}, afterFinish: function() {
            Draggable._dragging[e] = false
        }})
    }, zindex: 2000, revert: false, quiet: false, scroll: false, scrollSensitivity: 20, scrollSpeed: 15, snap: false, delay: 0};
    if (!arguments[1] || Object.isUndefined(arguments[1].endeffect)) {
        Object.extend(c, {starteffect: function(d) {
    ...
    Event.observe(this.handle, "e:mousedown", this.eventMouseDown);
    ...
    Event.stopObserving(this.handle, "e:mousedown", this.eventMouseDown);
    ...
    try {
        ...
        if ((tag_name = d.tagName.toUpperCase()) && (tag_name == "INPUT" || tag_name == "SELECT" || tag_name == "OPTION" || tag_name == "BUTTON" || tag_name == "TEXTAREA")) {
            return
        }
        if (!Object.isUndefined(b.memo) && !Object.isUndefined(b.memo.position)) {
            var c = b.memo.position
        } else {
            var c = [Event.pointerX(b), Event.pointerY(b)]
        }
        if (!c[0] && !Object.isUndefined(b.touches)) {
            var c = [b.touches[0].clientX, b.touches[0].clientY]
        }
    ...
    if (this.options.quiet) {
        ...
        if (!e[0] && !Object.isUndefined(b.touches[0])) {
            var e = [b.touches[0].clientX, b.touches[0].clientY]
        }
        if (!e[0] && !Object.isUndefined(b.changedTouches)) {
            var e = [b.changedTouches[0].clientX, b.changedTouches[0].clientY]
        }
    ...
    var g = false;
    if (f) {
        if (this.options.manualDropOff) {
            Droppables.prepareDelayedDropOff(this.element, b)
        } else {
    ...
    Droppables.reset();
    this.element.fire("drag:endDragging")
}, keyPress: function(a) {
...
Draggable._dragging = {};
var SortableObserver = Class.create({initialize: function(b, a) {
    this.element = $(b);
    this.observer = a;
...
var Sortable = {SERIALIZE_RULE: /^[^_\-](?:[A-Za-z0-9\-\_]*)[_](.*)$/, sortables: {}, _findRootElement: function(a) {
